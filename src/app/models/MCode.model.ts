  ...
  private seedInstance() {
    if (this.IMCodeInstance.mcode_raw != null) {
      ...
      do {
        ...
        if (mcode_match != null) {
          this.IMCodeInstance.mcode_raw = this.IMCodeInstance.mcode_raw.replace(
            mcode_match[0],
            '~',
          );
      ...
      if (this.IMCodeInstance.mcode_fields.length > 0) {
        this.IMCodeInstance.mcode_fields.forEach((mcode_field: string) => {
          this.IMCodeInstance.mcode_seeded =
            this.IMCodeInstance.mcode_seeded.replace(
              '{' + mcode_field + '}',
              this.IMCodeInstance.mcode_parent + "['" + mcode_field + "']",
            );
        });
      }

      this.IMCodeInstance.mcode_result =
        this.IMCodeInstance.mcode_error_default;
    } else {
  ...
  evaluateInstance(meval_type: string, meval_params: any): Promise<any> {
    if (this.IMCodeInstance.mcode_seeded != '') {
      let field_logic_code: string =
        `({
        evaluateBoolean: (` +
        this.IMCodeInstance.mcode_params +
        `): string => {
      ...
      try {
        eval(ts.transpile(field_logic_code))
          [meval_type](meval_params)
          .then((logic_result: boolean) => {
